...
- (BOOL)isDateInSelectedRange:(NSDate *)date; //from checkin to checkout
...
- (void)initialize
{
    ...
    if (!self.minimumDate) {
        _minimumDate = [_calendar startOfDayForDate:[NSDate date]];
    }
    if (!self.maximumDate) {
        _maximumDate = [[_calendar startOfDayForDate:[NSDate date]] fs_dateByAddingYears:1];
    }
...
- (void)layoutSubviews
{
    ...
    if (_needsLayoutForWeekMode) {
        ...
        if (_needsAdjustingMonthPosition) {
            ...
            _supressEvent = NO;
            NSInteger sectionNumber = [self monthsFromDate:[self beginingOfMonthOfDate:_minimumDate] toDate:_maximumDate]; //Bugfix for only one section layout Y offset abnormal
            if (sectionNumber) {
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    if (self.floatingMode) {
        if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
            ...
            separatorLine.backgroundColor = [OIAppTheme colorForSeparatorLine];
            [sectionFooter addSubview:separatorLine];
            [separatorLine mas_makeConstraints:^(MASConstraintMaker *make) {
                make.width.equalTo(sectionFooter);
                make.bottom.equalTo(sectionFooter);
...
//        return NO; //allow selected date should be selected once
    }
    ...
    if (_headerHeight == FSCalendarAutomaticDimension) {
        if (_preferedWeekdayHeight == FSCalendarAutomaticDimension) {
            if (!self.floatingMode) {
                ...
                _preferedHeaderHeight = FSCalendarStandardHeaderHeight * MAX(1, FSCalendarDeviceIsIPad * 1.5) * kDeviceScaleFactor;
            }
        }
...
- (CGFloat)preferedRowHeight
{
    if (_preferedRowHeight == FSCalendarAutomaticDimension) {
        ...
        if (!self.floatingMode) {
            ...
            _preferedRowHeight = FSCalendarStandardRowHeight * kDeviceScaleFactor * MAX(1, FSCalendarDeviceIsIPad*1.5);
        }
    }
...
- (BOOL)isDateInSelectedRange:(NSDate *)date {
    
    if (self.checkInDate && self.checkOutDate) {
        if ([date fs_daysFrom:self.checkInDate] >= 0 && [date fs_daysFrom:self.checkOutDate]<= 0) {
            return true;
        }
    }
    return false;
}
- (BOOL)isDateOutMaxAvalibleSelectRange:(NSDate *)date {
    
    if (_dataSource && [_dataSource respondsToSelector:@selector(isDateOutMaxAvalibleSelectRange:)]) {
        return [_dataSource isDateOutMaxAvalibleSelectRange:date];
    }
    return false;
}
...
- (void)reloadDataForCell:(FSCalendarCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.checkIn  = [cell.date fs_isEqualToDateForDay:_checkInDate];
    cell.checkOut = [cell.date fs_isEqualToDateForDay:_checkOutDate];

  
    cell.inSelectedRange = [self isDateInSelectedRange:cell.date];
...
//    cell.subtitle  = [self subtitleForDate:cell.date];
    ...
    switch (_scope) {
        case FSCalendarScopeMonth: {
            ...
            cell.dateIsPlaceholder = ![self date:cell.date sharesSameMonthWithDate:month] || ![self isDateInRange:cell.date] || [self isDateOutMaxAvalibleSelectRange:cell.date];
...
- (void)didSelectDate:(NSDate *)date
{
    ...
    if ([self.delegate respondsToSelector:@selector(calendar:willSelectDate:)]) {
        [self.delegate calendar:self willSelectDate:date];
    }
...
{
    return [self stringFromDate:date format:kDateFormatYYMMDD];
}
